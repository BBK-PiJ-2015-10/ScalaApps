package practice

import org.apache.spark.SparkConf
import org.apache.spark.SparkContext
import org.apache.spark.sql.SQLContext
import org.apache.spark.sql._

case class Auction(auctionid: String, bid: Float, bidtime: Float,
  bidder: String, bidderrate: Int, opebid: Float, price: Float,
  item: String, daystolive: Int
)


object SQLExample extends App {
  
  val configuration = new SparkConf().setAppName("SparkSQLUseCase").setMaster("local[*]")
  
  val sparkContext = new SparkContext(configuration)
  
  val sqlContext = new SQLContext(sparkContext)
  
  import sqlContext.implicits._
  import sqlContext._

  val ebayText = sparkContext.textFile("auctions/*.csv")
  
  val ebay = ebayText.map(_.split(",")).map(p=>Auction(p(0),
    p(1).toFloat,p(2).toFloat,p(3),p(4).toInt,p(5).toFloat,
    p(6).toFloat,p(7),p(8).toInt))
  
    
  //println(ebay.count())
  
  val auction = ebay.toDF
  
  auction.show()
  
  //auction.printSchema()
  
  //auction.select("auctionid").distinct().count()
  
  //auction.groupBy("auctionid","item").count().show()
  
  //val highPrice = auction.filter("price > 100")
  
  //highPrice.show()
  
  auction.registerTempTable("ATABLE")
  
  /*
   
  var results = sqlContext.sql(
  """ SELECT auctionid, item, count(bid) FROM ATABLE
     GROUP BY auctionid, item """     
  )
  
  results.show()
  
  */
  
  var results2 = sqlContext.sql(
  """
    SELECT auctionid, MAX(price) FROM ATABLE    
    GROUP BY item, auctionid
  """    
  )
  
  results2.show()
  
}